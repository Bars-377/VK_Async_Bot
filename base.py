...
class base:
    ...
    async def readiness_status(*args):
        ...
        try:
            ...
            def get_status_text(state_1, state_2):
                status_map = {
                    ("Обработка", "Ещё не получен"): "Документы в обработке в МФЦ",
                    ("Обработано", "Ещё не получен"): "Документы в обработке в МФЦ",
                    ("Невостребованный результат", "Ещё не получен"): "Документы переданы в архив ведомства",
                    ("Выдача результата", "Исполнено"): "Документы находятся на выдаче в МФЦ",
                    ("Выдача результата", "Отказ"): "Документы находятся на выдаче в МФЦ",
                    ("Отправлено курьером", "Ещё не получен"): "Документы в обработке в ведомстве",
                    ("Отправлено ЭВ", "Ещё не получен"): "Документы в обработке в ведомстве",
                    ("Отправлено курьером и ЭВ", "Исполнено"): "Документы в обработке в ведомстве",
                    ("Отправлено курьером и ЭВ", "Промежуточные результаты от ведомства"): "Документы в обработке в ведомстве",
                }
                return status_map.get((state_1, state_2))

            if validate_code(caseNumber):
                # Получаем первый статус
                data_1 = fetch_status(status_url_1.format(caseNumber))
                state_name_1 = data_1.get("state_name")
                request_id = data_1.get("request_id")

                # Получаем второй статус
                data_2 = fetch_status(status_url_2.format(request_id))
                state_name_2 = data_2.get("result_state_history", [{}])[0].get("state_name")
                address = data_2.get("created_mfc_name")

            else:
                # Получаем второй статус
                data_2 = fetch_status(status_url_2.format(caseNumber))
                state_name_2 = data_2.get("result_state_history", [{}])[0].get("state_name")
                request_id = data_2.get("id")

                # Получаем первый статус
                data_1 = fetch_status(status_url_1.format(request_id))
                state_name_1 = data_1.get("state_name")
                address = data_1.get("created_mfc_name")

            # Определяем текстовый статус
            res = get_status_text(state_name_1, state_name_2)

            if res:
                result = {"code": "found", "status": res, "address": address}
            else:
                result = {"code": "operator", "status": "", "address": ""}

        except requests.RequestException as e:
            print(f"Ошибка HTTP-запроса: {type(e).__name__}: {str(e)}")
            result = {"code": "error", "status": ""}
        except (KeyError, IndexError, ValueError) as e:
            print(f"Ошибка обработки данных: {type(e).__name__}: {str(e)}")
            result = {"code": "error", "status": ""}
        except Exception as e:
    ...
    #         if validate_code(caseNumber):
    #             status_url = status_url_1.format(caseNumber)
    #             response = requests.get(status_url, timeout=(2, 5))
    #             response.encoding = 'utf-8'
    #             xstatus_1 = response.json()

    #             # print(xstatus)
    #             state_name_1 = xstatus_1.get("state_name")

    #             request_id = xstatus_1.get("request_id")

    #             status_url = status_url_2.format(request_id)
    #             response = requests.get(status_url, timeout=(2, 5))
    #             response.encoding = 'utf-8'
    #             xstatus_2 = response.json()

    #             result_state_history = xstatus_2.get("result_state_history")
    #             state_name_2 = result_state_history[0].get("state_name")

    #             res = None

    #             if state_name_1 == "Обработка" and state_name_2 == "Еще не получен":
    #                 res = "Документы в обработке в МФЦ"
    #             elif state_name_1 == "Обработано" and state_name_2 == "Еще не получен":
    #                 res = "Документы в обработке в МФЦ"
    #             elif state_name_1 == "Невостребованный результат" and state_name_2 == "Еще не получен":
    #                 res = "Документы переданы в архив ведомства"
    #             elif state_name_1 == "Выдача результата" and state_name_2 == "Исполнено":
    #                 res = "Документы находятся на выдаче в МФЦ"
    #             elif state_name_1 == "Выдача результата" and state_name_2 == "Отказ":
    #                 res = "Документы находятся на выдаче в МФЦ"
    #             elif state_name_1 == "Отправлено курьером" and state_name_2 == "Еще не получен":
    #                 res = "Документы в обработке в ведомств"
    #             elif state_name_1 == "Отправлено ЭВ" and state_name_2 == "Еще не получен":
    #                 res = "Документы в обработке в ведомств"
    #             elif state_name_1 == "Отправлено курьером и ЭВ" and state_name_2 == "Исполнено":
    #                 res = "Документы в обработке в ведомств"
    #             elif state_name_1 == "Отправлено курьером и ЭВ" and state_name_2 == "Промежуточные результаты от ведомства":
    #                 res = "Документы в обработке в ведомств"
    ...
    #         else:
    #             status_url = status_url_2.format(caseNumber)
    #             response = requests.get(status_url, timeout=(2, 5))
    #             response.encoding = 'utf-8'
    #             xstatus_2 = response.json()
    ...
    #             xstatus_2_id = xstatus_2.get("id")

    #             status_url = status_url_1.format(xstatus_2_id)
    #             response = requests.get(status_url, timeout=(2, 5))
    #             response.encoding = 'utf-8'
    #             xstatus_1 = response.json()
